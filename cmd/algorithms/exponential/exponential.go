...
func parseArgs(ctx *cli.Context) (int, int, error) {
	...
	if ctx.Args().Len() != 2 {
		// early exit
		return 0, 0, errors.New("requires 2 arguments BASE POWER")
	}
	...
	if val, err := strconv.Atoi(ctx.Args().Get(1)); err != nil {
		...
		returnErr = errors.Join(returnErr, errors.New("args[1]: POWER should be a positive integer"))
	} else {
		in1 = val
	}
...
var RootCmd = &cli.Command{
	...
	Action: func(ctx *cli.Context) error {
		...
		fmt.Printf("%d^%d = %d\n", in0, in1, algorithms.Exponential(in0, in1))
		return nil
	},
}
